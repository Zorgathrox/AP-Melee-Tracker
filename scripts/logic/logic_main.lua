...
super_smash_bros._melee_location = {}
super_smash_bros._melee_location.__index = super_smash_bros._melee_location
...
function super_smash_bros._melee_location.new(name)
    local self = setmetatable({}, super_smash_bros._melee_location)
    if name then
...
function super_smash_bros._melee_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = super_smash_bros._melee_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function super_smash_bros._melee_location:connect_two_ways(exit, rule)
...
function super_smash_bros._melee_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function super_smash_bros._melee_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function super_smash_bros._melee_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function super_smash_bros._melee_location:accessibility()
...
-- 
function super_smash_bros._melee_location:discover(accessibility, keys)
...
entry_point = super_smash_bros._melee_location.new("entry_point")
